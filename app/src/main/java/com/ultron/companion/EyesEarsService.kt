...
import com.ultron.companion.system.WakeWordDetector
import java.util.concurrent.Executors
import kotlin.concurrent.thread

/**
 * "Eyes and ears": while running, periodically grabs a still frame from
 * the phone's camera, streams short chunks of raw mic audio, and runs
 * wake-word detection — all forwarded to ULTRON via UltronClient.
 */
class EyesEarsService : LifecycleService() {
    ...
    override fun onCreate() {
        ...
        if (hasMic) {
            startAudio()
            // WakeWordDetector uses SpeechRecognizer which must run on the
            // main thread — we are on it here (onCreate is main thread).
            wakeWordDetector = WakeWordDetector(this, "ultron") {
                Log.d(TAG, "Wake word detected")
                update("wake word!")
                client.sendCommand("wake_word_detected")
            }
            wakeWordDetector?.start()
            Log.d(TAG, "WakeWordDetector started")
        } else {
            Log.w(TAG, "RECORD_AUDIO not granted - ears and wake word disabled, eyes only")
        }
        update("live")
    }
    ...
    private fun startAudio() {
        ...
        val chunkBytes = (AUDIO_SAMPLE_RATE * (AUDIO_CHUNK_MS / 1000.0) * 2).toInt()
        val recorder = try {
            AudioRecord(MediaRecorder.AudioSource.MIC, AUDIO_SAMPLE_RATE, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, maxOf(minBuf, chunkBytes))
